...
const header=document.createElement("div");header.className="route-header";header.textContent=route+" Jobs";jobsContainer.appendChild(header);
const line=document.createElement("div");line.className=route==="Commoner"?"commoner-line":"military-line";jobsContainer.appendChild(line);
jobs.filter(j=>j.route===route).forEach(job=>{
if(job.unlockReq){const reqJob=jobs.find(j=>j.id===job.unlockReq.jobId);if(reqJob.level>=job.unlockReq.level)job.unlocked=true;}
if(job.unlockReq&&jobs.find(j=>j.id===job.unlockReq.jobId).unlocked===false){jobDiv=null;return;}
if(route==="Military"&&job.id===4){job.unlocked=false;}
const jobDiv=document.createElement("div");jobDiv.className="job "+(job.unlocked?"unlocked":"locked");
...
const popup=document.createElement('div');popup.className='popup';popup.textContent=text;document.body.appendChild(popup);
const offset=activePopups.length*60;popup.style.bottom=`${20+offset}px`;activePopups.push(popup);
requestAnimationFrame(()=>popup.classList.add('show'));
setTimeout(()=>{popup.classList.remove('show');setTimeout(()=>{popup.remove();activePopups=activePopups.filter(p=>p!==popup);activePopups.forEach((p,i)=>p.style.bottom=`${20+i*60}px`);},500);},1500);}
function update(time){const delta=time-lastTime;lastTime=time;let gps=0;jobs.forEach(job=>{if(job.running){job.progress+=delta/job.duration;if(job.progress>=1){job.progress=0;gold+=Math.round(job.goldPerRun);job.xp+=10;if(job.xp>=job.xpNeeded){job.xp=0;job.level++;job.goldPerRun=Math.round(job.goldPerRun*1.1);job.xpNeeded=Math.round(job.xpNeeded*1.2);showPopup(`${job.emoji} ${job.name} leveled up to ${job.level}!`);}showPopup(`+${Math.round(job.goldPerRun)} Gold`);updateXpBar(job);}const progressBar=document.getElementById(`progress-${job.id}`);if(progressBar)progressBar.style.width=`${job.progress*100}%`;gps+=job.goldPerRun/(job.duration/1000);}
const jobDiv=document.getElementById(`progress-${job.id}`).parentElement.parentElement;
const tooltip=jobDiv.querySelector('.tooltip');if(tooltip)tooltip.textContent=`Gold: ${job.goldPerRun} | XP: ${job.xp}/${job.xpNeeded} | Duration: ${(job.duration/1000).toFixed(1)}s`;
const titleEl=jobDiv.querySelector('.job-title');titleEl.innerHTML=`<span>${job.unlocked?job.emoji:"❓"}</span> ${job.unlocked?job.name:"Locked"} (Lv ${job.level})`;});
